...
> 3. **Widget Config API 為強依賴**：`GET /api/v1/widget/config` 為正式依賴，Widget 初始化必須呼叫；`welcomeMessage`、`disclaimer`、`fallbackMessage` 為多語系文字物件 `Record<string, string>`；`quickReplies` 為多語系字串陣列物件 `Record<string, string[]>`；`status` 值：`online | offline | degraded`；失敗觸發降級模式
> 4. **Dashboard 本期納入**：後台 Dashboard 頁面、`AdminStatCard`、圖表元件、`GET /api/v1/admin/dashboard` API 均為本期正式範圍；`services/api/admin/dashboard.ts` 本期建立；最少回傳欄位：對話數、Lead 數、Ticket 數、常見意圖、轉接原因、近期稽核事件
...
> **Quick Replies 分層決策**：
>
> 本期 quickReplies 分為兩類：
>
> 1. **Global / Welcome Quick Replies**
...
>
> 2. **Per-message Quick Replies**
>    - 顯示於每一則 AI 回覆訊息下方
>    - 資料來源為 `ChatMessageVM.quickReplies?: string[]`
>    - 未來由 chat SSE done event / message metadata 回傳
>    - 不由 `/admin/widget-settings` 管理
>    - 不建立獨立 `/admin/quick-replies` 頁面管理
>
> ---
>
> **【本期 API Contract 原則】**
>
> 本期前端正式 API contract 採資源式 `/api/v1/...` 路由。前端以 `sessionToken` 作為匿名訪客會話識別，並透過 **path parameter** 呼叫 session-scoped API。聊天回覆正式採 `fetch + ReadableStream` 接收 SSE 串流，事件格式統一為 `token / done / error / timeout / interrupted`。Widget Config、Lead、Handoff、Feedback、Ticket、Dashboard API 皆以本文件定義之 schema 為準。本期不做 Auth / Login / RBAC、Email 通知、handoff status API。
...
| 狀態管理 | Pinia |
| 表單驗證 | vee-validate |
| 串流 | **SSE（Server-Sent Events）**（本期確認使用，`fetch` + `ReadableStream`，不使用 `EventSource`） |
| 圖表 | Nuxt Charts |
...
/admin                  → 自動導向 /admin/dashboard（本期首頁）
/admin/dashboard        → 後台 Dashboard 概覽
...
/admin/intent          → 意圖 / 模板管理
/admin/widget-settings  → Widget 文案與設定管理（含 welcomeMessage / quickReplies / disclaimer / fallbackMessage）
/admin/audit            → 稽核事件列表
...
**admin domain（後台）**
- 知識庫、對話紀錄、Lead/Ticket、意圖/模板、Widget 設定、稽核、回饋紀錄
- 預設 / 全域 / 歡迎引導 quickReplies 併入 Widget 設定管理，不作為獨立 admin domain。
...
後台（含左側導覽）
  ...
  │   ├─ 列表
  │   └─ 新增 / 編輯
  ├─ Widget 設定（含 welcomeMessage / quickReplies / disclaimer / fallbackMessage 與即時預覽）
  ├─ 稽核事件
  │   ├─ 列表
  │   └─ 詳情
  ├─ 回饋紀錄
...
- 狀態燈顏色：綠色（`online`）、紅色（`degraded`）、灰色（`offline`）
...
**Layer 1 — Global / Welcome Quick Replies（`ChatQuickReplies` 元件）**
- 顯示於新 session 開始時的歡迎訊息下方，或 Message Area 與 Input Bar 之間的全域快捷提問列
- 資料來源：`useWidgetConfigStore.config?.quickReplies`
- 後台管理位置：`/admin/widget-settings`
- API 來源：`GET /api/v1/widget/config`
- 型別：`Record<string, string[]>`
- 只在對話重置後或第一次開啟時顯示
- 點擊後直接以使用者身份送出訊息，同時全域快捷提問列收起
- 不由 `/admin/quick-replies` 管理

**Layer 2 — Per-message Quick-reply Chips（`AiMessageItem` 內嵌）**
- 資料來源：`ChatMessageVM.quickReplies?: string[]`
- 附加於特定 AI 訊息氣泡下方
- 點擊後等同使用者輸入該文字送出
- Phase 2+ 應由 chat SSE done event / message metadata 回傳
...
串流完成後：`ai-streaming` type 轉換為 `ai` type，渲染完整 Markdown 內容，並顯示 Feedback 按鈕。
...
後端標記某則回覆的信心度低於門檻時，`ai` 訊息底部附加 `SystemLowConfidenceItem` inline 提示條，文案：「以上回覆僅供參考，建議直接聯繫客服確認」，附聯絡捷徑按鈕。此提示與 Feedback 按鈕共存，不互相干擾。

### 5.9 留資表單卡片（Lead Form Card）

觸發條件：訪客主動詢問報價 / 要求聯絡 / AI 主動引導時，由後端回應中帶入特殊 type，前端於對話串中渲染 `LeadFormCard`。亦可由轉人工流程引導觸發。

表單設計：
- 表單以白色卡片形式嵌入訊息流中
- **必填：姓名（name）、Email（email）**
- **選填：公司名稱（company）、電話（phone）、備注 / 詢問訊息（message）**
- `language`：由前端自動帶入當前語系（`zh-TW` / `en`），不需訪客填寫，隨表單提交
- 欄位即時驗證（失焦後觸發）
- 提交按鈕：所有必填完成後啟用
...
後端建立 Lead 紀錄
  ↓
前端於訊息流中顯示靜態確認訊息：
「已轉交專人協助，我們將盡快與您聯繫。如需提供聯絡方式，請填寫下方表單。」
  ↓（可選）引導顯示 LeadFormCard
```

- 轉人工後 Input Bar 不鎖定（訪客可繼續輸入問題）
- 無需 polling `GET /api/v1/chat/handoff/status`，不更新「等待接線」等多段狀態

**本期不做（移至未來規劃）**：
- HandoffStatusCard 多狀態流轉（requested / waiting / connected / unavailable）
- Handoff status polling API
- `useHandoff` composable 的 polling 邏輯
- `handoff-unavailable` 狀態下顯示服務時間

**`handoff-status` 訊息 type** 本期不渲染，前端改以普通 `system` 型訊息顯示確認文案。
...
3. Widget Config API 呼叫失敗（網路錯誤或非 2xx 回應）

進入降級模式後：
- **Widget 仍可見、仍可展開**（Launcher 保持可點擊）
- Launcher 顯示紅色（`degraded`）或灰色（`offline`）狀態燈 + 降級文案
...
**本期實作（正式串接 Feedback API）**：
- 點擊讚 / 倒讚：呼叫 `useFeedback.submitFeedback(sessionToken, messageId, value, reason?)`，向 `POST /api/v1/chat/sessions/:sessionToken/messages/:messageId/feedback` 送出回饋，payload `{ value: 'up'|'down', reason? }`
- 同時更新 store 中對應訊息的 `rating` 欄位（樂觀更新）
- 再次點擊相同值：重置為 `null`（取消評分）
- 點擊倒讚後可展開原因選填（選填）
- 回饋採 **fire-and-forget**，API 失敗不影響主對話流，僅記錄 console error
...
本期 Ticket 的狀態流定義為四態：`open`（開啟）→ `in_progress`（進行中）→ `resolved`（已解決）→ `closed`（已關閉），不做更複雜的 escalation 流程。
...
若後端保留 `status`，前端可讀取或顯示，但本期 Widget Settings 表單核心以以上四個 key 為主。CTA 文案、AI 標記文字、線上 / 離線 / 降級文案、聯絡捷徑等項目移至未來擴充。
...
> **注意**：本期前台 Feedback API 正式串接（`POST /api/v1/chat/sessions/:sessionToken/messages/:messageId/feedback`，payload `{ value: 'up'|'down', reason? }`），後台回饋紀錄列表資料由後端管理提供（`GET /api/v1/admin/feedback`）。
...
| `ChatMessageArea` | 訊息滾動區域；component registry map 動態渲染各型訊息元件；**無訊息時顯示 Empty State**（Bot avatar + Widget Config welcomeMessage + Global / Welcome quickReplies） |
| `ChatQuickReplies` | 全域快捷提問按鈕列（Layer 1）；資料來源為 Widget Config API 的 `quickReplies: Record<string, string[]>` |
...
| 元件名稱 | 說明 |
|---------|------|
| `LeadFormCard` | 留資表單 inline 卡片（name + email 必填，company + phone 選填） |
| `HandoffStatusCard` | 轉人工確認訊息卡片（本期只顯示「已轉交專人協助」靜態訊息；多狀態流程 waiting / connected / unavailable **本期不做**，移至未來規劃） |
...
| 用途 | Phase 1 mock 遺留，暫時模擬後台知識資料庫並提供 `query()` 關鍵字比對；不作為正式資料來源 |
...
| 移除條件 | Phase 2 `POST /api/v1/chat/message` SSE 串接完成後，移除整個 store；`useChat.sendMessage` 改呼叫真實 API；Global / Welcome quickReplies 由 Widget Config API 提供；Per-message quickReplies 由 chat SSE done event / message metadata 提供 |
...
本期設計以 **SSE（Server-Sent Events）** 為主（本期確認使用 `fetch + ReadableStream`，不使用 `EventSource`）。

SSE 事件格式：
- `event: token` → `data: {"token":"..."}` — 逐字輸出 token
- `event: done` → `data: {"messageId":"uuid","action":"answer|handoff|fallback|intercepted","sourceReferences":[],"usage":{"promptTokens":0,"completionTokens":0,"totalTokens":0,"durationMs":0,"model":"string","provider":"string"}}` — 串流完成
...
串流取消：呼叫 `AbortController.abort()`，不使用 HTTP DELETE endpoint。

串流流程：
1. 使用者送出訊息
2. 呼叫 `POST /api/v1/chat/sessions/:sessionToken/messages`，帶入訊息內容
3. 後端回傳 SSE stream，前端以 `fetch + ReadableStream` 接收
4. 每收到一個 `token` event，append 至 `AiStreamingItem` 的內容
5. 收到 `done` event 時，結束串流，依 `action` 欄位觸發對應 UI（handoff / fallback / intercepted）
6. 收到 `error` / `timeout` / `interrupted` event 時，觸發對應錯誤 UI 狀態
...
Widget 展開
  ↓
讀取 localStorage 的 sessionToken（key: 'chat_session_token'）
  ├─ 有 token → GET /api/v1/chat/sessions/:sessionToken/history
  │     ├─ 200 OK → 載入歷史訊息，進入 active 狀態
  │     └─ 401/404 → 清除舊 token，走「無 token」流程
  └─ 無 token → POST /api/v1/chat/sessions（建立新 session）
                  ├─ 200 OK → 儲存新 sessionToken 至 localStorage，顯示歡迎訊息
                  └─ 失敗 → 進入 fallback mode
```

`sessionToken`：前端唯一識別符，儲存於 localStorage，透過 **path parameter** 傳入 session-scoped API（`/api/v1/chat/sessions/:sessionToken/...`）。後端以此映射至內部 `sessionId`，前端不直接操作 `sessionId`。

### 9.4 Widget Config 載入時機

**Widget 初始化時**（Widget 第一次掛載到 DOM）呼叫 `GET /api/v1/widget/config`。本 API 為本期正式確認之強依賴（Hard Dependency）。

設計決策：每次展開 Widget 時重新取得 Config（不快取至 localStorage），確保文案更新後使用者能即時看到最新設定。若 API 失敗或回傳 `status: 'offline'`，觸發降級模式（fallback）。

Config 提供欄位：`status`（`online | offline | degraded`）、`welcomeMessage: Record<string, string>`、`quickReplies: Record<string, string[]>`、`disclaimer: Record<string, string>`、`fallbackMessage: Record<string, string>`。前端依當前 locale 從物件中取對應語系值。
...
使用者點擊送出
  ↓
前端表單驗證（name 必填、email 必填且格式正確；company / phone / message 選填）
  ├─ 驗證失敗 → 顯示欄位錯誤，不送出
  └─ 驗證成功 → POST /api/v1/chat/sessions/:sessionToken/lead（payload: { name, email, company?, phone?, message?, language? }）
...
本期採用**簡化版轉人工流程**（不做 polling 或 SSE status event）：

```
觸發轉人工
  ↓
POST /api/v1/chat/sessions/:sessionToken/handoff
  ├─ 200 OK → 後端建立 Lead → 回傳 { accepted, action, leadId?, ticketId?, message } → 前端顯示靜態「已轉交專人協助」訊息
  └─ 失敗 → 顯示錯誤提示，允許重試
```

`HandoffStatusCard` 的多狀態流程（waiting / connected / unavailable）**本期不實作，移至未來規劃**。
...
開啟 ImportModal，選擇 CSV / JSON 檔案
  ↓
POST /api/v1/admin/knowledge/import（multipart/form-data）
  ↓
後端同步解析並回傳結果：
...
降級期間，**Widget 仍可見、仍可展開**，但前台 Widget 的所有 AI 功能停用，聯絡捷徑與留資入口保持可用。每 60 秒重新探測服務狀態，自動恢復。
...
### 11.3 QuickRepliesMap

```
Record<string, string[]>     // { 'zh-TW': ['產品規格'], en: ['Product specs'] }
```
...
```
{
  ...
  company?: string           // 選填
  phone?: string             // 選填
  message?: string           // 選填（備注/詢問訊息）
  language?: string          // 由前端自動帶入當前語系（如 'zh-TW'、'en'），不需訪客填寫
  sessionToken: string       // 關聯 session（透過 path parameter 傳入）
}
```
...
```
{
  status: 'idle' | 'requesting' | 'transferred'
  // 本期只用 idle → requesting → transferred 三態
  // waiting / connected / unavailable 多狀態流程本期不做，移至未來規劃
}
```
...
│   │   │   │   ├── useHandoff.ts
│   │   │   │   ├── useLeadForm.ts
│   │   │   │   └── useFeedback.ts         # 本期正式串接 Feedback API（POST /api/v1/chat/sessions/:sessionToken/messages/:messageId/feedback）
│   │   │   └── stores/
│   │   │       ├── chatWidget.ts
│   │   │       ├── chatSession.ts
│   │   │       └── widgetConfig.ts
...
│   │   ├── index.vue                 # 前台入口
│   │   └── admin/
│   │       ├── index.vue             # redirect → /admin/dashboard
│   │       ├── dashboard.vue
│   │       ├── knowledge/
│   │       │   ├── index.vue
│   │       │   ├── new.vue
...
│   │   │       ├── audit.ts
│   │   │       ├── feedback.ts
│   │   └── streaming.ts              # SSE 串流處理（本期確認採用 SSE）
│   │
...
**Phase 1（核心聊天體驗，最優先）**：
- 前台 Widget 完整互動（收合 / 展開 / 問答 / **SSE 串流**（本期確認）/ 快捷提問 / 攔截提示 / 降級）
- Session 建立與 localStorage 恢復（`sessionToken`）
- Widget Config 載入（`GET /api/v1/widget/config`，本期正式強依賴）與 fallback

**Phase 2（前台功能完整化）**：
- 留資表單（LeadFormCard，name + email 必填）
- 轉人工流程（**簡化版**：POST handoff → 顯示靜態「已轉交」訊息；多狀態 HandoffStatusCard 移至未來規劃）
- 滿意度回饋：**本期正式串接 Feedback API**（`POST /api/v1/chat/sessions/:sessionToken/messages/:messageId/feedback`），payload `{ value: 'up'|'down', reason? }`，`useFeedback.ts` 呼叫 API，fire-and-forget，失敗不影響主流
- 多語系切換（英文）

**Phase 3（後台基礎建設）**：
- 對話紀錄查詢（列表 + 詳情）
- Lead 管理
- **Dashboard**：統計卡片列 + 對話量趨勢圖 + 意圖分布圓餅圖 + 最新稽核事件
- **Ticket 管理**：列表 + 詳情 + 狀態流（`open → in_progress → resolved → closed` 四態）+ 狀態更新（`PATCH .../status`）+ 備註新增（`POST .../notes`）

**Phase 4（後台內容管理）**：
- 知識庫管理（含版本歷史、批次匯入）
- 意圖 / 模板管理
- Widget 設定管理（含 welcomeMessage / quickReplies / disclaimer / fallbackMessage 與即時預覽）

**Phase 5（後台維運工具）**：
- 稽核事件列表與詳情
- 回饋紀錄（`GET /api/v1/admin/feedback`，本期 Feedback API 正式串接，資料由後端提供）
- **營運報表本期不做，移至未來規劃**

以上 Phase 1–4 的功能均在本期設計範圍內（已標記「本期不做」者除外）。Copilot 生成 `plan.md` / `task.md` 時，應將全部 Phase 的任務納入，並標記已明確 deferred 的項目。
...
| # | 待確認項目 | 影響 | 預設方案 |
|---|-----------|------|---------|
| ~~R1~~ | ~~後端串流協議：SSE 或 WebSocket？~~ | ~~前台 `useStreaming.ts` 實作方式~~ | **已確認：SSE，本期採用 SSE 串流，不考慮 WebSocket。** |
| R2 | Widget 嵌入形式：直接 script embed / iframe / Web Component？ | Widget bundle 打包方式與 CSS 隔離策略 | 本期以 Nuxt 頁面形式開發，嵌入形式 TBD |
| R3 | 前台 session token 儲存：localStorage 還是 HttpOnly Cookie？ | XSS 風險評估、跨域嵌入相容性 | 預設 localStorage（`chat_session_token`） |
| R4 | 轉人工後接入的外部客服系統為何？ | `useHandoff.ts` 的未來多狀態擴充複雜度 | **本期已簡化：POST handoff → 靜態訊息，不做 polling / status event** |
...
| R9 | Ticket 狀態流與 API 契約 | Phase 3 工作量 | **已確認：本期納入 Ticket（開啟 → 進行中 → 已關閉）** |
