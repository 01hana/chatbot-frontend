...
| `ai-text` | AI 純文字回覆（支援 Markdown） | `AiMessageItem` |
| `ai-streaming` | AI 串流中的回覆（逐字動畫） | `AiStreamingItem` |
| `ai-product-card` | AI 回覆含產品卡片 | `AiProductCardItem` |
...
| `lead-form` | 留資表單卡片（inline） | `LeadFormCard` |
| `handoff-status` | 轉人工狀態卡片（inline） | `HandoffStatusCard` |
...
每則 `ai-text` 訊息底部右側顯示 👍 / 👎 icon button（灰色預設）。

狀態：
- 點擊讚：讚 button 變藍，倒讚 button 維持灰色；即時呼叫回饋 API
- 點擊倒讚：倒讚 button 變紅，同時展開原因選填（TBD：chips 選項清單 或 單行文字輸入）
- 已回饋後可切換（讚 → 倒讚，或倒讚 → 讚），切換時再次呼叫 API 更新
- 同一訊息每次只能有一個有效回饋

回饋 API 採 fire-and-forget，失敗不影響主對話流，但在 console 記錄錯誤。
...
| 元件名稱 | 說明 |
|---------|------|
| `ChatWidget` | Widget 根元件，管理收合 / 展開狀態 |
| `ChatLauncher` | 收合態 Launcher（膠囊 / FAB） |
| `ChatPanel` | 展開態聊天面板容器 |
| `ChatHeader` | Header（品牌 / 狀態 / 語系切換 / 關閉） |
| `ChatInfoBar` | 聯絡捷徑列 |
| `ChatMessageArea` | 訊息滾動區域 |
| `ChatQuickReplies` | 快捷提問按鈕列 |
...
| `AiMessageItem` | AI 完整回覆氣泡（含 Markdown 渲染） |
| `AiStreamingItem` | AI 串流中氣泡（逐字動畫） |
| `AiProductCardItem` | AI 產品卡片訊息 |
...
| R5 | 滿意度倒讚原因：chips 選項 or 開放文字？ | `MessageFeedback` 元件設計 | 預設 chips（4–5 個選項） |
