...
> 3. **Widget Config API 為強依賴**：`GET /api/v1/widget/config` 為正式依賴，Widget 初始化必須呼叫；`welcomeMessage`、`quickReplies`、`disclaimer`、`fallbackMessage` 均為多語系物件 `{"zh-TW":"...","en":"..."}`；`status` 值：`online|offline|degraded`；失敗觸發降級模式
...
| 移除條件 | Phase 2 `POST /api/v1/chat/message` SSE 串接完成後，移除整個 store；`useChat.sendMessage` 改呼叫真實 API；`useChatSession._appendWelcomeMessage` 改由 Widget Config API 提供 quickReplies |
...
Config 提供欄位（均以多語系物件形式回傳）：`status`（`online|offline|degraded`）、`welcomeMessage`（`{"zh-TW":"...","en":"..."}`）、`quickReplies`（`{"zh-TW":[...],"en":[...]}`）、`disclaimer`（多語系物件）、`fallbackMessage`（多語系物件）。前端依當前 locale 從物件中取對應語系值。
